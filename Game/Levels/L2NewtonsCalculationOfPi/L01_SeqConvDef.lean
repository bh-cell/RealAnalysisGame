...
Without further ado, here it is: Given a sequence `a : ℕ → ℝ` and a real number `L : ℝ`, we say that the sequence `a` **converges** in the limit to some real number `L`,
 if:
...
But over time, I hope you'll come to see that it
 embodies a beautiful negotiation between precision and effort. I like to think of it as a conversation between an engineer and a factory worker. The engineer arrives with specifications: 'I need the output to be within `ε` of the target value `L`.' The factory worker responds, 'Sure, I can achieve that tolerance, but you'll need to let me  run the machines for at least `N` hours to guarantee it. You're welcome to run them for longer than `N` hours, but as long as you give me at least that much, I'll forever after guarantee the specified tolerance.' The smaller the tolerance `ε` the engineer demands from the start, the larger `N` typically needs to be -- precision comes at a cost!

What makes this definition so powerful is its universality. The factory worker is essentially promising: 'Give me any tolerance requirement, no matter how stringent, and I can meet it -- though I might need more resources (larger `N`) for tighter specifications.'
...
`c`, and a hypothesis that, for all values of `n`, we have  `a (n) = c`; then prove that `a` does converge, and converges to `c`. This is the simplest possible case: if our 'factory' always produces the exact target value `c`, then we can meet any tolerance requirement immediately!
...
You will see the goal as `SeqLim a c`. What are you supposed to do with that,
...
`change ∀ ε > 0, ∃ N : ℕ, ∀ n ≥ N, |a n - c| < ε`

Lean will then change the goal to look like this.
Remember that `ε`, `N`, and `n` are all dummy variables
here, so you can have some fun:

`change ∀ Alice > 0, ∃ Bob : ℕ, ∀ blah ≥ Bob, |a blah - c| < Alice`
...
**CAUTION** Remember how Lean *must* have space after a function name, it won't accept `f(x)` but instead requires `f (x)`? Well... it's the other way around for absolute
values. Lean won't accept a space after an absolute value.
So if you write `| a n - c|`, you'll get an error message.
Same with `|a n - c |` -- the space at the end is the problem. Sorry! I didn't write the syntax.
...
/--
For any sequence `a : ℕ → ℝ` and constant `c : ℝ`, and
hypothesis `h : ∀ n, a n = c`, the theorem `ConstLim`
proves that `SeqLim a c`, that is, the (constant) sequence `a` converges to `c`.
-/
TheoremDoc ConstLim as "ConstLim" in "Theorems"


/-- Prove that the constant sequence converges to its constant value.
This is the factory worker's dream scenario: we're already producing exactly what's required! -/
Statement ConstLim (a : ℕ → ℝ) (c : ℝ) (a_const : ∀ n, a n = c) : SeqLim a c := by
  Hint (hidden := true) "Try starting with `change ∀ ε > 0, ∃ N : ℕ, ∀ n ≥ N, |a n - c| < ε`"
  change ∀ ε > 0, ∃ N : ℕ, ∀ n ≥ N, |a n - c| < ε
  intro ε hε
  use 1
  intro n hn
  ...
  norm_num
  Hint (hidden := true) "While you may see `0 < ε` in the goal and `ε > 0` in the hypothesis `hε`, Lean will still know that these two things are `exact`ly the same..."
  exact hε
...
You showed that if a sequence always outputs the same value `c`, then it converges to `c`. The factory worker's response to any tolerance demand `ε > 0` is beautifully simple: 'I can meet that specification immediately with any production run length `N`, because I'm already producing exactly what you want!'

**Key Insights from this proof:**

1. **The `change` tactic**: You learned how to unfold a definition to see what you're really trying to prove. `SeqLim a c` became the concrete epsilon-N condition.
...
2. **The logical structure**: The proof followed the natural flow of the definition:
   ...
   - Then algebraic manipulation showed that `|a n - c| = |c - c| = 0 < ε`

**The Beautiful Simplicity:**
This is the factory worker's dream scenario—no matter how demanding the engineer's tolerance requirements, the constant factory can satisfy them instantly. There's no trade-off between precision and effort because the output is already perfect!
...
**Proof:** Suppose we have a sequence $a(n)$ where $a(n) = c$ for all $n$, and we want to show that this sequence converges to $c$.

By definition, we need to show that for any tolerance $\\varepsilon > 0$, we can find a point $N$ such that for all $n \\geq N$, we have $|a(n) - c| < \\varepsilon$.

This is almost trivially simple: since $a(n) = c$ for every $n$, we have:
$$|a(n) - c| = |c - c| = |0| = 0$$
...
Therefore, for any $n \\geq N$, we have $|a(n) - c| = 0 < \\varepsilon$, which proves convergence.
**QED**
